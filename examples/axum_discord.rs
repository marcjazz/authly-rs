...
async fn main() {
    ...
    let client_id =
        std::env::var("AUTHLY_DISCORD_CLIENT_ID").expect("AUTHLY_DISCORD_CLIENT_ID must be set");
...
async fn discord_login(State(state): State<AppState>, cookies: Cookies) -> impl IntoResponse {
    initiate_oauth_login(&state.discord_flow, &cookies, &["identify", "email"])
}
